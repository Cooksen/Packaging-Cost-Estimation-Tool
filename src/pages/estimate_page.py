...
#         df_report = pd.DataFrame(component_rows)
...
#             "Quantity": np.nan,
#             "Unit Price (USD)": np.nan,
#             "Total Price (USD)": df_report["Total Price (USD)"].sum(),
#         }
...
#         st.success(f"{df_report.iloc[-1]['Total Price (USD)']:.2f} USD")
...
def render():
    ...
    def render_corrugate_box(title, key_prefix):
        st.header(title)
        weight = st.number_input("Weight (g)", min_value=0.0, key=f"{key_prefix}_weight")
        qty = st.number_input("Quantity", min_value=0, key=f"{key_prefix}_qty")
        return {"name": title, "material": "Corrugate", "weight": weight, "qty": qty}
    ...
    for i, comp in enumerate(st.session_state.custom_components):
        with st.expander(f"Custom Component #{i+1}", expanded=True):
            comp["name"] = st.text_input("Component Name", comp["name"], key=f"name_{i}")
            comp["material"] = st.selectbox("Material Type", ["Corrugate", "MPP", "EPE"], key=f"material_{i}")
            comp["weight"] = st.number_input("Weight (g)", min_value=0.0, key=f"weight_{i}")
            comp["qty"] = st.number_input("Quantity", min_value=0, key=f"qty_{i}")
            custom_inputs.append(comp)
            if st.button(f"❌ Remove this component", key=f"delete_{i}"):
                deleted_indices.append(i)
    ...
    if st.button("💰 Estimate Total Cost"):
        ...
        add_row("System Box", "Corrugate", sb_info["weight"], sb_info["qty"], corrugate_model)
        add_row("Accessory Box", "Corrugate", ac_info["weight"], ac_info["qty"], corrugate_model)
        add_row("KB Tray", "Corrugate", kb_info["weight"], kb_info["qty"], corrugate_model)
        add_row("EPE", "EPE", epe_weight, epe_qty, epe_model)
        add_row("MPP", "MPP", mpp_weight, mpp_qty, mpp_model)
        ...
        df_material = pd.DataFrame([row for row in component_rows if row["Material"] != "Freight"])
        ...
        if OD > 0:
            freight_price = freight_model.predict(np.array([[OD]]))[0]
            st.success(f"Based on the entered dimensions, the box volume is {OD:.5f} m³, resulting in a predicted freight cost of ${freight_price:.2f}.")
        else:
            freight_price = 0
        ...
        towrite = BytesIO()
        df_report.to_excel(towrite, index=False, sheet_name="CostEstimate", engine="openpyxl")
        towrite.seek(0)
        ...
        st.subheader("✅ Estimated Total Price")
        st.success(f"Material cost: {material_total:.2f} + Freight cost: {freight_price:.5f} = {material_total + freight_price:.2f} USD")